...
class Config:
    ...
    front_z_history_frames: int = 3
    front_z_hold_invalid_frames: int = 1
...
class StepHeightDetectorNode(Node):
    """ROS2 node that preserves the verified D435 measurement pipeline.

    Published topics:
      /step_detector/boundary_detected : std_msgs/Bool
      /step_detector/height_valid      : std_msgs/Bool
      /step_detector/height_m          : std_msgs/Float32
      /step_detector/front_z_m         : std_msgs/Float32

    The D435 is opened directly through pyrealsense2. Do not run
    realsense2_camera for the same physical camera at the same time.
    """

    def __init__(self) -> None:
        super().__init__("step_height_detector")

        try:
            import pyrealsense2 as rs
        except ImportError as exc:
            raise RuntimeError(
                "pyrealsense2 is required. Verify that realsense-viewer "
                "and the original standalone detector work on this Jetson first."
            ) from exc

        self.rs = rs
        self.cfg = Config()
        self.boundary_tracker = BoundaryTracker(self.cfg)
        self.height_tracker = HeightTracker(self.cfg)
        self.front_z_history: Deque[float] = deque(
            maxlen=self.cfg.front_z_history_frames
        )
        self.front_z_pair: Optional[np.ndarray] = None
        self.front_z_invalid_frames = 0
        ...
        self._last_status_print_s = 0.0
        self.show_roi = True

        self.boundary_pub = self.create_publisher(
            Bool, "/step_detector/boundary_detected", 10
        )
        self.valid_pub = self.create_publisher(
            Bool, "/step_detector/height_valid", 10
        )
        self.height_pub = self.create_publisher(
            Float32, "/step_detector/height_m", 10
        )
        self.z_pub = self.create_publisher(
            Float32, "/step_detector/front_z_m", 10
        )
        self.reset_measurement_service = self.create_service(
            Trigger,
            "/step_detector/reset_measurement_session",
            self._on_reset_measurement_session,
        )
        ...
        self.spatial, self.temporal = configure_depth_filters(rs)
        self._closed = False

        self.timer = self.create_timer(
            1.0 / float(self.cfg.fps),
            self.process_frame,
        )

        self.get_logger().info(
            f"{BUILD} started: D435 "
            f"{self.cfg.width}x{self.cfg.height}@{self.cfg.fps} FPS"
        )
        self.get_logger().info(
            "Publishing /step_detector/boundary_detected, "
            "/step_detector/height_valid, /step_detector/height_m, "
            "/step_detector/front_z_m"
        )
    ...
    def _reset_front_z(self) -> None:
        self.front_z_history.clear()
        self.front_z_pair = None
        self.front_z_invalid_frames = 0

    def _reset_measurement_state(self) -> None:
        self.height_tracker.reset()
        self._reset_front_z()

    def _on_reset_measurement_session(
        self,
        _request: Trigger.Request,
        response: Trigger.Response,
    ) -> Trigger.Response:
        self._reset_measurement_state()
        self._publish_bool(self.valid_pub, False)
        self._publish_float(self.height_pub, float("nan"))
        self._publish_float(self.z_pub, float("nan"))
        response.success = True
        response.message = "Height measurement session reset"
        self.get_logger().info("Height measurement session reset completed")
        return response

    def _update_front_z(
        self,
        confirmed: Optional[BoundaryPair],
        detected: Optional[BoundaryPair],
        evidence: Optional[PairEvidence],
    ) -> float:
        current_is_valid = (
            confirmed is not None
            and detected is not None
            and self.boundary_tracker.current_matches(detected)
            and evidence is not None
            and evidence.tier >= self.cfg.min_evidence_tier
            and np.isfinite(evidence.face_z_m)
            and self.cfg.depth_min_m
            <= evidence.face_z_m
            <= self.cfg.depth_max_m
        )
        if not current_is_valid:
            self.front_z_invalid_frames += 1

            if (
                self.front_z_history
                and self.front_z_invalid_frames
                <= self.cfg.front_z_hold_invalid_frames
            ):
                held_z = float(np.median(self.front_z_history))
                self.get_logger().warning(
                    f"D435 front_z transient invalid; "
                    f"holding recent Z={held_z:.3f}m"
                )
                return held_z

            self.get_logger().warning(
                "D435 front_z invalid beyond hold window; publishing NaN"
            )
            self._reset_front_z()
            return float("nan")

        self.front_z_invalid_frames = 0
        current_pair = pair_array(detected)
        if (
            self.front_z_pair is not None
            and not pair_is_close(current_pair, self.front_z_pair, self.cfg)
        ):
            self.front_z_history.clear()

        self.front_z_pair = current_pair
        self.front_z_history.append(float(evidence.face_z_m))
        return float(np.median(self.front_z_history))
    ...
    def _print_status(
        ...
        if now_s - self._last_status_print_s < STATUS_PRINT_PERIOD_S:
            return
        self._last_status_print_s = now_s

        height_text = "---"
        z_text = "---"
        if is_valid and measurement is not None:
            height_text = f"{measurement.height_m * 100.0:.2f} cm"
        if np.isfinite(front_z_m):
            z_text = f"{front_z_m * 100.0:.1f} cm"

        timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
        print(
            f"[{timestamp}] "
            f"boundary={str(boundary_detected):<5} | "
            f"valid={str(is_valid):<5} | "
            f"height={height_text:<9} | Z={z_text}",
            flush=True,
        )

    def process_frame(self) -> None:
        # poll_for_frames() avoids blocking the ROS2 executor.
        ...
        if self.cfg.use_realsense_filters:
        ...
        confirmed = self.boundary_tracker.update(detected)
        current_front_z_m = self._update_front_z(
            confirmed,
            detected,
            detected_evidence,
        )
        ...
        if (
            ...
            and detected_evidence.tier >= self.cfg.min_evidence_tier
        ):
            measure_pair_array = (
        ...
        stable_measurement = self.height_tracker.update(raw_measurement)

        # Boundary state is published every processed frame.
        self._publish_bool(
            self.boundary_pub,
            confirmed is not None,
        )

        # height_valid applies only to the stabilized height measurement.
        is_valid = stable_measurement is not None
        self._publish_bool(self.valid_pub, is_valid)

        if stable_measurement is not None:
            self._publish_float(
                self.height_pub,
                stable_measurement.height_m,
            )

        # Approach distance is current-frame data and is independent of the
        # height tracker. NaN explicitly invalidates a missing/stale distance.
        self._publish_float(self.z_pub, current_front_z_m)
        ...
        if self.show_gui:
            ...
            cv2.imshow(
                "D435 - Two Boundaries Median-Y Height [ROS2]",
                display,
            )
